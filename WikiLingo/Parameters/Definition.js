...
rules: [/^(?:('))/,/^(?:("))/,/^(?:.*?(?=(')))/,/^(?:.*?(?=(")))/,/^(?:('))/,/^(?:("))/,/^(?:([a-zA-Z0-9_-]+)(?=(([=]|[=][>]))))/,/^(?:(([=]|[=][>])))/,/^(?:\s)/,/^(?:$)/],
conditions: {"singleQuoteParameter":{"rules":[0,2,4,5,6,7,8,9],"inclusive":true},"doubleQuoteParameter":{"rules":[1,3,4,5,6,7,8,9],"inclusive":true},"INITIAL":{"rules":[4,5,6,7,8,9],"inclusive":true}}
};
return lexer;
})();
parser.lexer = lexer;
